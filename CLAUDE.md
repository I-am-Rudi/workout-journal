...
An Obsidian plugin that tracks workouts, exercises, and fitness progress. Users create exercise/routine/plan notes in their vault, run live workout sessions in an interactive sidebar view, and the plugin saves session logs as markdown files with YAML frontmatter.
...
There is no test suite. TypeScript type checking is the only automated check: `npx tsc --noEmit --skipLibCheck`.

**To use the plugin locally:** copy `main.js`, `manifest.json`, and `styles.css` into an Obsidian vault's `.obsidian/plugins/workout-journal/` folder, then reload Obsidian.

## Releasing

Releases are triggered by pushing a git tag (any tag, e.g. `v0.1.23`). The CI workflow (`main.yml`) runs `version-bump.mjs` (updates `manifest.json`, `versions.json`, `package.json`), builds, commits the version bump back to main, then publishes a GitHub Release with the three artifact files.

To bump the version locally before tagging:

```bash
npm version patch   # or minor/major — runs version-bump.mjs via the "version" script
```
...
All data lives in the Obsidian vault as markdown files with `wj-*` YAML frontmatter keys. The `wj-type` field discriminates the four note types: `exercise`, `routine`, `plan`, `workout`.

- **Exercise notes** — define an exercise (muscle groups, defaults, last-performed values). Stored in `exerciseLibraryFolder`.
- **Routine notes** — ordered list of exercises with per-set targets. Stored in `routinesFolder`.
- **Plan notes** — ordered list of routines (a training block). Stored in `workoutPlansFolder`.
- **Workout logs** — immutable record of a finished session. Stored in `defaultWorkoutFolder`.

All four folder paths (and a performance CSV path) are configurable in settings.

### Services (src/utils/)

| Service | Responsibility |
|---|---|
| `DefinitionFileService` | CRUD for exercise/routine/plan notes; resolves exercise references in routines |
| `WorkoutFileService` | Save/load workout log notes; bi-directional sync between frontmatter and readable markdown body |
| `WorkoutSessionService` | Creates a `WorkoutSession` from a routine (pre-filling targets from CSV history), converts finished session back to a `Workout` log, merges session changes back into a routine |
| `PerformanceCsvService` | Appends completed sets to a CSV file; used to look up historical performance when pre-filling session targets |

### Session lifecycle (src/plugin.ts → src/views/WorkoutSessionView.ts)

1. User triggers a session start (from routine, plan, or current note).
2. `WorkoutSessionService.createSessionFromRoutine()` builds a `WorkoutSession` with pre-filled targets sourced from the CSV history.
3. `WorkoutSessionView` (an Obsidian `ItemView`) renders the live session UI, opening in a popout window on desktop.
4. On finish, `plugin.finishActiveSession()` saves the workout log, appends to the CSV, optionally updates `lastPerformedReps`/`lastPerformedWeight` in exercise notes, and optionally merges changes back into the routine note.

### Frontmatter auto-sync

When `enableAutoSyncFrontmatter` is true, a debounced `vault.on("modify")` listener detects manual edits to workout log files and re-parses the markdown body to keep the `wj-*` frontmatter keys in sync. The body is the source of truth for sync; frontmatter is the source of truth for plugin reads.

### Entry point

`main.ts` re-exports the default from `src/plugin.ts`. `WorkoutTrackerPlugin` (extends `Plugin`) registers all commands, the ribbon icon, the settings tab, and the `WorkoutSessionView`.

### Key types (src/types/index.ts)

- `Workout` / `Exercise` / `ExerciseSet` — the persisted log model
- `ExerciseDefinition` / `RoutineDefinition` / `WorkoutPlanDefinition` — the library/template model
- `WorkoutSession` / `WorkoutSessionExercise` / `WorkoutSessionSet` — the in-memory live-session model
- `WorkoutTrackerSettings` — settings shape (defaults in `src/settings/defaults.ts`)

### Note templates

Users can add custom frontmatter and body content to each note type via `settings.noteTemplates`. `noteTemplateUtils.ts` provides `parseTemplateFrontmatter` and `appendTemplateBody` helpers that both services use when rendering notes — user-supplied keys are merged in first so that plugin-managed `wj-*` keys always take precedence.

### Role

You are a contributor to this repository. You are expected to understand the
architecture and conventions described above, and to follow them when adding new
features or fixing bugs. Please ensure that any changes you make are consistent
with the existing codebase and adhere to best practices for TypeScript and
Obsidian plugin development. You adhere to the philosophy of the project:
providing a dedicated app like performance while staying note-centric and
compatible with Zettelkasten workflows. You are expected to write clear,
maintainable code and to document your changes appropriately.
